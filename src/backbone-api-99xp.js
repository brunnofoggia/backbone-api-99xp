...
//  might be necessary to overwrite it
    setAuthHeader(o) {
        if(!this.tokenField) return BackboneApi.dataError(`tokenField is not set`);
...
//        !method && (method = this.options.method);
        ...
        var vErr = this.validate(this.getMethodInput(methodData), {methodData: methodData});
        if(vErr !== null) return this.validationErrors(vErr);
        ...
        if(methodData.public) return calledFn();
        else if(!this.auth) return BackboneApi.dataError(`method "${method}" is not set as public but auth is not set`);
    ...
    callApi(method, methodData, success, error) {
        ...
        var before = _.bind(typeof methodData.before === 'function' ? methodData.before : (c, o) => { c(o); }, this),
            save = _.bind(_.partial((method, methodData, success, error, o) => {
                this.listenToOnce(this, 'sync', _.bind(()=>{
                    this.data[method] = this.attributes;
                    this.attributes = {};

                    try {
                        typeof methodData.success === 'function' && _.bind(methodData.success, this)(o, this._req.body, methodData);
                        typeof success === 'function' && _.bind(success, this)(o, this._req.body, methodData);
                    } catch(e) {
                        console.error('Internal Failure');
                        console.log(e); 
                        this._res.status(500).send({message: 'Internal Failure'});
                    }
                }, this));
                this.listenToOnce(this, 'error', _.bind(()=>{
                    !this._reqErr.response && console.error('Internal Failure');
                    try {
                        typeof methodData.error === 'function' && _.bind(methodData.error, this)(this._reqErr.response, o, this._req.body, methodData);
                        typeof error === 'function' ? _.bind(error, this)(this._reqErr.response || null, o, this._req.body, methodData) : !this._res._headerSent && this._res.status(this._reqErr.response ? this._reqErr.response.status : 500).send(this._reqErr.response ? this._reqErr.response.data : null);
                    } catch(e) {
                        console.error('Internal Failure');
                        console.log(e); 
                        this._res.status(500).send({message: 'Internal Failure'});
    ...
    methodData(method) {
        ...
        if(!methodData.path) BackboneApi.dataError("Make sure you've set a path for method '"+method+"'");
    ...
    setHttpMethod(methodData) {
        ...
        methodData = _.defaults(methodData, {method: (typeof data === 'object' && _.size(data)>0 ? 'POST' : 'GET') });
    ...
    setApiUrl(path, methodData) {
        var host = _.result(this, 'urlHost') || BackboneApi.urlError(1),
            data = _.extend({}, _.clone(this.data), {_model: this, _input: this.getMethodInput(methodData), _params: this._req.params});
        
        path = _.template(path)(data);
        return this.urlRoot = [host, path].join('/').replace(/((?<!\:)\/{1,})/g, '/');
    },
    ...
    validations(a, o) {
        var v = _.result2(o.methodData, 'validations', {}, [a, o], this);
        return v;
    },
    _validate() {
        return true;
    ...
    setRouterParameters(req, res) {
        if (!req || !res) BackboneApi.dataError('Initialize requires an object with req and res (express route variables) within');
